...
def stages(w_s, w_total, nabla, p01, h01,gamma,pic):
    w_s1 = .7 * w_s
    w_s2 = .85 * w_s
    w_sn = .9 * w_s
    n = var('n')
    ...
    w.append(w_s1)
    w.append(w_s2)
    rest_of_work = (w_total - (w_s1 + w_s2 + w_sn)) / (num_of_stages - 3)
    for i in range(num_of_stages - 3):
        w.append(rest_of_work)
    w.append(w_sn)
    h = []
    for i in range(num_of_stages):
        h.append(h01)
        h01 += w[i]
    #print('Enthalpy at each stage',h)




    nabla_list = []
    for i in range(num_of_stages-2):
        nabla_list.append(nabla)

    #print('pressure efficeincy',nabla_list)

    p = [p01]

    for i in range(len(nabla_list)):
        eff_comp = (1 + nabla_list[i] * ((w[i]) / (h[i]))) ** (gamma / (gamma - 1))
        p.append(eff_comp * p01)
        p01 = p[i+1]

    #print('Pressure at each Stage',p)
    # solving for final efficiency
    eff = var('eff')

    # print(h[-2])
    # last_pressure = (1 + eff * ((w[-2]) / (h[-2]))) ** (gamma / (gamma - 1)) * p01
    # eq = Eq(last_pressure,pic)
    # sol = solve(eq, eff)
    # print('sol', sol)
    ...
    for i in range(len(h)):
        ...
        t.append(GP.T(h[i]))
    #print('Temp at each stage',t)
    for i in range(len(p)):
        s.append(GP.s(T= t[i], p = p[i]))

    print(s)


    return w,h,p,t, num_of_stages
...
values = stages(25,w_total,eff,p01,h01,gamma,pic)
work = values[0]
enthalpy = values[1]
pressure = values[2]
temperature = values[3]
stage_amount = values[4]
print('Work:',work)
print('Enthalpy:',enthalpy)
print('Pressure:',pressure)
print('Temperature:',temperature)
print('Stage Amount:',stage_amount)
...
def iterate_cu(stage_number,delta_ca, R, cu, ca,w, u):
...
#
# pressure_eff(nabla,p01,works,h,p_a_comp,1.4)
#
#
